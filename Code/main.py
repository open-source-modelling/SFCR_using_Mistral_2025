"""Entry point for batch SFCR table extraction across all companies."""
...
def _get_api_key() -> str:
    """Read the Mistral API key from the ``MISTRAL_API_KEY`` environment variable."""
...
def _run_table_for_companies(
    table: str,
    runner: Callable[[str, str, pd.DataFrame], pd.DataFrame],
    companies: np.ndarray,
    api_key: str,
    master_list: pd.DataFrame,
) -> pd.DataFrame:
    """Run one table extractor for every company and concatenate results by column."""
    frames = []
    for company in companies:
        company_frame = runner(company=company, api_key=api_key, master_list=master_list)
...
def main():
    """Load the master list, extract each supported table, and write CSVs to ``Output/``."""
    ...
    for table in table_list:
        ...
        if runner is None:
            ...
            continue
        combined = _run_table_for_companies(
            table=table,
            runner=runner,
