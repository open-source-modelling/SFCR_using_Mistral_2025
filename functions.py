...
def run_assets_by_sections(company:str, unique_id:str, api_key:str, master_list:pd.DataFrame)->pd.DataFrame:
    ...
    output = pd.concat([output_02_A_1, output_02_A_2, output_02_A_3, output_02_A_4, output_02_A_5, output_02_A_6], axis=0)
...
def run_S230101_first_half(company:str, unique_id:str, api_key:str, master_list:pd.DataFrame)->pd.DataFrame:
    ...
    annotations_response = call_mistral(client=client, output_pdf_path = output_pdf_path, pydantic_model = class_output)
    extracted_data = class_output(**json.loads(annotations_response.document_annotation))
    output_1 = extracted_to_df(extracted_data, company)

    class_output = OwnFundsDeductions
    annotations_response = call_mistral(client=client, output_pdf_path = output_pdf_path, pydantic_model = class_output)
    extracted_data = class_output(**json.loads(annotations_response.document_annotation))
    output_2 = extracted_to_df(extracted_data, company)
    
    class_output = OwnFundsAuxiliaryOwnFunds
    annotations_response = call_mistral(client=client, output_pdf_path = output_pdf_path, pydantic_model = class_output)
    extracted_data = class_output(**json.loads(annotations_response.document_annotation))   
    output_3 = extracted_to_df(extracted_data, company)
...
def run_S230101_first_half_ita(company:str, unique_id:str, api_key:str, master_list:pd.DataFrame)->pd.DataFrame:
    ...
    class_output = OwnFundsDeductionsIta
    annotations_response = call_mistral(client=client, output_pdf_path = output_pdf_path, pydantic_model = class_output)
    extracted_data = class_output(**json.loads(annotations_response.document_annotation))
    output_2 = extracted_to_df(extracted_data, company)
    
    class_output = OwnFundsAuxiliaryOwnFundsIta
    annotations_response = call_mistral(client=client, output_pdf_path = output_pdf_path, pydantic_model = class_output)
    extracted_data = class_output(**json.loads(annotations_response.document_annotation))   
    output_3 = extracted_to_df(extracted_data, company)
...
def run_company_S_23_01_01(company:str, api_key:str, master_list:pd.DataFrame)->pd.DataFrame:
    ...
    for id in relevant_rows.index:    
        if relevant_rows.loc[id,"type"] == "B":
            out_1 = run_S230101_first_half(company, id, api_key, master_list)
            out_2 = run_S230101_second_half(company, id, api_key, master_list)
        elif relevant_rows.loc[id,"type"] == "B_ITA":
            out_1 = run_S230101_first_half_ita(company, id, api_key, master_list)
            out_2 = run_S230101_second_half_ita(company, id, api_key, master_list)
        elif relevant_rows.loc[id,"type"] == "A":
            out_1 = run_S230101_first_half(company, id, api_key, master_list)     
        elif relevant_rows.loc[id,"type"] == "L":
            out_2 = run_S230101_second_half(company, id, api_key, master_list)
    ...
    return pd.concat([out_1, out_2], axis=0)    
